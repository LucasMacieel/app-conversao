...
class MatplotFigure(Widget):
            ...
            if np.nanmin(distance) < dp(50):
                ...
                if self.compare_xdata:
                    ...
                    if len(idx_best_list) > 0:
                        ...
                        if self.hover_instance.x_hover_pos > self.x + self.axes.bbox.bounds[2] + self.axes.bbox.bounds[
                            0] or \
                                self.hover_instance.x_hover_pos < self.x + self.axes.bbox.bounds[0] or len(
                            index_list) == nb_widget:
        ...
        if self.hover_instance:
            ...
            # update hover pos if needed
                ...
                if self.hover_instance.x_hover_pos > self.x + self.axes.bbox.bounds[2] + self.axes.bbox.bounds[0] or \
                        self.hover_instance.x_hover_pos < self.x + self.axes.bbox.bounds[0] or \
                        self.hover_instance.y_hover_pos > self.y + self.axes.bbox.bounds[1] + self.axes.bbox.bounds[
                    3] or \
                        self.hover_instance.y_hover_pos < self.y + self.axes.bbox.bounds[1]:
                    self.hover_instance.hover_outside_bound = True
                else:
    ...
    def apply_zoom(self, scale_factor, ax, anchor=(0, 0), new_line=None):
        ...
        if self.do_zoom_x:
            if scale == 'linear':
                ...
                try:
                    new_min, new_max = self.inv_transform_eval(new_min, ax.yaxis), self.inv_transform_eval(new_max,
                                                                                                           ax.yaxis)
        ...
        if self.do_zoom_y:
            if yscale == 'linear':
                ...
                try:
                    new_ymin, new_ymax = self.inv_transform_eval(new_ymin, ax.yaxis), self.inv_transform_eval(new_ymax,
                                                                                                              ax.yaxis)
                except OverflowError:  # Limit case
    ...
    def apply_pan(self, ax, event, mode='pan'):
        ...
        xpress, ypress = trans.transform_point(
            (self._last_touch_pos[event][0] - self.pos[0], self._last_touch_pos[event][1] - self.pos[1]))
        ...
        if not mode == 'pan_y' and not mode == 'adjust_y':
            if mode == 'adjust_x':
                ...
                if self.anchor_x == 'left':
                    if xdata > cur_xlim[0]:
                        if scale == 'linear':
                            ...
                            try:
                                cur_xlim = [
                                    self.inv_transform_eval((self.transform_eval(cur_xlim[0], ax.xaxis) - dx / 2),
                                                            ax.xaxis),
                                    self.inv_transform_eval((self.transform_eval(cur_xlim[1], ax.xaxis) - dx / 2),
                                                            ax.xaxis)]
                ...
                if scale == 'linear':
                    ...
                    try:
                        cur_xlim = [
                            self.inv_transform_eval((self.transform_eval(cur_xlim[0], ax.xaxis) - dx / 2), ax.xaxis),
        ...
        if not mode == 'pan_x' and not mode == 'adjust_x':
            if mode == 'adjust_y':
                ...
                if self.anchor_y == 'top':
                    if ydata > cur_ylim[0]:
                        if yscale == 'linear':
                            ...
                            try:
                                cur_ylim = [
                                    self.inv_transform_eval((self.transform_eval(cur_ylim[0], ax.yaxis) - dy / 2),
                                                            ax.yaxis),
                                    self.inv_transform_eval((self.transform_eval(cur_ylim[1], ax.yaxis) - dy / 2),
                                                            ax.yaxis)]
                            except (ValueError, OverflowError):
                ...
                if yscale == 'linear':
                    ...
                    try:
                        cur_ylim = [
                            self.inv_transform_eval((self.transform_eval(cur_ylim[0], ax.yaxis) - dy / 2), ax.yaxis),
    ...
    def reset_box(self):
        ...
        if min(abs(self._box_size[0]), abs(self._box_size[1])) > self.minzoom:
            ...
            self.x1_box, self.y1_box = trans.transform_point((self._box_size[0] + self._box_pos[0] - self.pos[0],
